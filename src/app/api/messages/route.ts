...
export async function GET(request: NextRequest) {
  try {
    ...
    // Check if we need to add an admin reply to Muneeb's message
    const muneebMessage = data.messages.find(msg => msg.email === 'muneeb76256@gmail.com')
    if (muneebMessage && (!muneebMessage.replies || muneebMessage.replies.length === 0)) {
      // Add admin reply to Muneeb's message
      if (!muneebMessage.replies) {
        muneebMessage.replies = []
      }
      muneebMessage.replies.push({
        id: Date.now().toString(),
        message: 'Thank you for your message, Muneeb! We have received your inquiry and will get back to you shortly with a detailed response.',
      ...
      const sampleMessages: Message[] = [
        {
          ...
          message: 'Hello! I really love your website. The design is amazing and very user-friendly.',
          timestamp: new Date(now.getTime() - 7200000).toISOString(), // 2 hours ago
          replies: [
            {
              id: '1',
              message: 'Thank you so much for your kind words! We really appreciate your feedback.',
              timestamp: new Date(now.getTime() - 6600000).toISOString(), // 1 hour 50 minutes ago
              sender: 'admin'
            }
          ]
        },
        {
          ...
          message: 'I have a question about your services. Do you offer custom solutions for businesses?',
          timestamp: new Date(now.getTime() - 5400000).toISOString(), // 1.5 hours ago
          replies: [
            {
              id: '2',
              message: 'Hi Sarah! Yes, we do offer custom solutions. Please contact us at business@example.com for more details.',
              timestamp: new Date(now.getTime() - 4800000).toISOString(), // 1 hour 20 minutes ago
              sender: 'admin'
            },
            {
              id: '3',
              message: 'Thank you for the quick response! I will send an email right away.',
              timestamp: new Date(now.getTime() - 4200000).toISOString(), // 1 hour 10 minutes ago
              sender: 'user'
            }
          ]
        },
        {
          ...
          message: 'Great work on the recent update! The new features are fantastic.',
        ...
        },
        {
          ...
          message: 'Is there a way to schedule a demo of your product? I\'m interested in learning more.',
          timestamp: new Date(now.getTime() - 1800000).toISOString(), // 30 minutes ago
          replies: [
            {
              id: '4',
              message: 'Hi Emily! Absolutely! You can schedule a demo by visiting our booking page or calling us directly.',
        ...
        },
        {
          ...
          message: 'Thank you for the excellent customer support! You resolved my issue quickly.',
          timestamp: new Date(now.getTime() - 600000).toISOString(), // 10 minutes ago
          replies: []
        }
      ]
      data.messages = sampleMessages
    }
