...
export function ModernSidebar() {
  ...
  const navigationItems = [
    ...
    { id: 'contact', name: 'Contact', icon: MessageSquare, href: '#contact', action: 'scroll' },
  ]

  const secondaryItems = [
    { id: 'settings', name: 'Settings', icon: Settings, href: '/settings', action: 'navigate' },
    { id: 'archive', name: 'Archive', icon: Archive, href: '/archive', action: 'navigate' },
    { id: 'starred', name: 'Starred', icon: Star, href: '/starred', action: 'navigate' },
    { id: 'trash', name: 'Trash', icon: Trash2, href: '/trash', action: 'navigate' },
  ]
  ...
  return (
    <ModernSidebarContext.Provider value={{ 
      ...
      <motion.div
        ...
        className="hidden md:flex md:flex-col bg-background/95 backdrop-blur-md border-r border-border h-screen fixed left-0 top-0 z-30"
      >
...
function SidebarItem({ item, isActive, onClick, isCollapsed }: any) {
  ...
  return (
    <motion.button
      ...
      className={`group relative flex items-center gap-3 p-3 rounded-lg transition-all duration-200 w-full text-left ${
        isActive 
